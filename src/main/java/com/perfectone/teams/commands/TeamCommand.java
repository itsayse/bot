...
public class TeamCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "allies" -> handleAllies(player);
    ...
    /** Requires the sender to own a team, and resolves the target team by name. Returns null (and messages the player) on failure. */
    private TeamData requireOwnedTeamAndTarget(Player player, String[] args, String usage) {
        ...
        if (!ourTeam.getOwner().equals(player.getUniqueId()) && !player.hasPermission("teams.admin")) {
            player.sendMessage(ChatColor.RED + "Only the team owner can manage alliances.");
            return null;
        }
        return ourTeam;
    }
    ...
    private void handleAlly(Player player, String[] args) {
        TeamData ourTeam = requireOwnedTeamAndTarget(player, args, "Usage: /team ally <team>");
        if (ourTeam == null) return;

        Optional<TeamData> targetOpt = plugin.getTeamManager().getTeam(args[1]);
        ...
        TeamManager.AllyResult result = plugin.getTeamManager().proposeAlly(ourTeam, targetTeam);
        switch (result) {
            case CANNOT_ALLY_SELF -> player.sendMessage(ChatColor.RED + "You can't ally your own team.");
            case ALREADY_ALLIED -> player.sendMessage(ChatColor.YELLOW + "You're already allied with "
                    + plugin.getTeamManager().colorizedPrefix(targetTeam) + ChatColor.YELLOW + ".");
            case NOW_ALLIED -> {
                Bukkit.broadcastMessage(ChatColor.AQUA + plugin.getTeamManager().colorizedPrefix(ourTeam) + ChatColor.AQUA
                        + " and " + plugin.getTeamManager().colorizedPrefix(targetTeam) + ChatColor.AQUA
                        + " are now allied - no points for kills between them.");
            }
            case REQUEST_SENT -> {
                player.sendMessage(ChatColor.GREEN + "Alliance request sent to " + plugin.getTeamManager().colorizedPrefix(targetTeam)
                        + ChatColor.GREEN + ". They need to run " + ChatColor.YELLOW + "/team ally " + ourTeam.getDisplayName()
                        + ChatColor.GREEN + " to accept.");
                Player targetOwner = Bukkit.getPlayer(targetTeam.getOwner());
                if (targetOwner != null) {
                    targetOwner.sendMessage(ChatColor.GREEN + plugin.getTeamManager().colorizedPrefix(ourTeam) + ChatColor.GREEN
                            + " has proposed an alliance! Run " + ChatColor.YELLOW + "/team ally " + ourTeam.getDisplayName()
                            + ChatColor.GREEN + " to accept.");
                }
            }
        }
    ...
    private void handleUnally(Player player, String[] args) {
        TeamData ourTeam = requireOwnedTeamAndTarget(player, args, "Usage: /team unally <team>");
        if (ourTeam == null) return;
        ...
        boolean wasAllied = plugin.getTeamManager().areAllied(ourTeam.getKey(), targetTeam.getKey());
        plugin.getTeamManager().removeAlly(ourTeam, targetTeam);

        if (wasAllied) {
            Bukkit.broadcastMessage(ChatColor.RED + plugin.getTeamManager().colorizedPrefix(ourTeam) + ChatColor.RED
                    + " has broken their alliance with " + plugin.getTeamManager().colorizedPrefix(targetTeam) + ChatColor.RED + ".");
        } else {
    ...
    private void handleAllies(Player player) {
        ...
        java.util.Set<String> allyKeys = plugin.getTeamManager().getAllies(team.getKey());
        if (allyKeys.isEmpty()) {
            player.sendMessage(ChatColor.YELLOW + "Your team has no allies yet.");
            return;
        }

        player.sendMessage(ChatColor.GOLD + "--- Allies of " + plugin.getTeamManager().colorizedPrefix(team) + ChatColor.GOLD + " ---");
        for (String allyKey : allyKeys) {
            plugin.getTeamManager().getTeam(allyKey).ifPresent(ally ->
                    player.sendMessage(plugin.getTeamManager().colorizedPrefix(ally)));
        }
    }
}
