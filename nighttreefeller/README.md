...
3. **Cycling tellraw announcements** — posts through a list of `/tellraw @a ...`
   messages, one every `interval-minutes` (default 20), looping back to the
...
announcements:
  tellraw:
    enabled: true
    interval-minutes: 20        # post the next message every 20 minutes
    initial-delay-minutes: 20   # wait 20 minutes after startup before the first one
    messages:
      - '{"text":"...json #1..."}'
      - '{"text":"...json #2..."}'
```

Each message runs verbatim as `tellraw @a <message>` from console, one per
`interval-minutes`, cycling back to the top of the list after the last one.
Add, remove, or reorder entries freely — each one just has to stay valid
JSON (test with Minecraft's own in-game `/tellraw` command, or any JSON
validator, if you're not sure). Set `enabled: false` to turn the whole thing
off.
