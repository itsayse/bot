...
5. On successful login (and every `key-check-interval-minutes`, default 5),
   ...
   revoked, the player is kicked with a message — and the bot strips the
   Discord role from their linked account around the same time (either
   immediately, on that check, or via the bot's periodic sweep).
...
Requires Maven and internet access to `repo.papermc.io`.
...
**Note:** I wasn't able to compile-test this in my sandbox (no access to
PaperMC's Maven repo or a JDK there), so please do a `mvn clean package` and
a smoke-test on your server before relying on it. The Paper version range in
`pom.xml` (`[26.2.build,)`) targets Paper's newer year-based versioning
(26.x) — if it doesn't resolve, check https://papermc.io/downloads/paper
for the current build and pin an exact version instead.
